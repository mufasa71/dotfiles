...
local fileTypeIconWidth = plenaryStrings.strdisplaywidth(devIcons.get_icon(
  "fname",
  { default = true }))
...
function telescopePickers.getPathAndTail(fileName)
  ...
  local pathWithoutTail = require("plenary.strings").truncate(fileName,
    #fileName -
    #bufferNameTail,
    "")
...
function telescopePickers.prettyFilesPicker(pickerAndOptions)
  ...
  Options.entry_maker = function(line)
    ...
    local displayer = telescopeEntryDisplayModule.create({
      ...
      items = {
        { width = fileTypeIconWidth }, { width = nil }, { remaining = true }
      }
    })
    ...
    originalEntryTable.display = function(entry)
      ...
      return displayer({
        { icon,          iconHighlight }, tailForDisplay,
...
function telescopePickers.prettyGrepPicker(pickerAndOptions)
  ...
  options.entry_maker = function(line)
    ...
    local displayer = telescopeEntryDisplayModule.create({
      separator = " ",                                                   -- Telescope will use this separator between each entry item
      items = {
        { width = fileTypeIconWidth }, { width = nil }, { width = nil }, -- Maximum path size, keep it short
        { remaining = true }
      }
    })
    ...
    originalEntryTable.display = function(entry)
      ...
      -- Get the Tail and the Path to display
      local tail, pathToDisplay =
          telescopePickers.getPathAndTail(entry.filename)
      ...
      local text = options.file_encoding and
          vim.iconv(entry.text, options.file_encoding, "utf8") or
          entry.text
      ...
      return displayer({
        { icon,          iconHighlight }, tailForDisplay,
        { pathToDisplay, "TelescopeResultsComment" }, text
      })
    end
